...
On systems without a valid IPv6 configuration (like on some Contabo vservers), unbound has to be configured to not use IPv6 explicitly. In such cases the `cloudron-setup` script will fail midway and unbound logs from `journalctl -u unbound` will contain:
```
error: can't bind socket: Cannot assign requested address for ::1
```
IPv6 support can be disabled with:
```
